...
def create_help_request(question, caller_id):
    ...
    conn.commit()
    conn.close()
    print(f"[Mock LiveKit] Text to supervisor: Hey, I need help answering '{question}' for caller {caller_id} (Request ID: {request_id})")
    return request_id
...
def resolve_help_request(request_id, answer):
    ...
    conn.commit()
    conn.close()
    print(f"[Mock LiveKit] Text to caller {request['caller_id']}: Here's the answer: {answer}")
...
def check_timeouts():
    ...
    for request in timed_out:
        ...
        print(f"[Mock LiveKit] Text to caller {request['caller_id']}: Sorry, we couldn't answer '{request['question']}' in time.")
    conn.commit()
    conn.close()